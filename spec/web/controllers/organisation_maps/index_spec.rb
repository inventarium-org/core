...
RSpec.describe Web::Controllers::OrganisationMaps::Index, type: :action do
  ...
  let(:all_communications) { ->(*) { Success([Communication.new(id: 123)]) } }
  ...
  context 'when user authenticated' do
    ...
    context 'and organisation_operation returns success result' do
      ...
      context 'and communication operation returns success result' do
        let(:all_communications) { ->(*) { Success([Communication.new(id: 321)]) } }
        ...
        it do
          ...
          expect(action.communications).to eq([Communication.new(id: 321)])
        end
      end

      context 'and communication operation returns failure result' do
        let(:all_communications) { ->(*) { Failure(:something) } }
        ...
        it do
          subject
          expect(action.organisation).to eq(Organisation.new(id: 123))
          expect(action.communications).to eq([])
        end
      end
    end
